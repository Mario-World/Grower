...
class Executor(AgentExecutor):
 ...
 async def execute(self,context,q):
  ...
  await q.enqueue_event(new_text_status_update_event(task_id=task.id,context_id=task.context_id,state=TaskState.TASK_STATE_WORKING,text="Writing personalized outreach..."))
  try:
   prompt=f"""Create concise personalized outreach for each prospect using ONLY supplied facts.
Return ONLY JSON: {{"messages":[{{"company":"","subject":"","email":"","cta":"Book a demo"}}]}}
Never invent people, metrics, events or capabilities. Mention the observed signal naturally.
INPUT:
{raw[:30000]}"""
  ...
  await q.enqueue_event(new_text_artifact_update_event(task_id=task.id,context_id=task.context_id,name="outreach",text=out)); await q.enqueue_event(new_text_status_update_event(task_id=task.id,context_id=task.context_id,state=TaskState.TASK_STATE_COMPLETED,text=out))
 async def cancel(self,context,q): pass
@click.command()
@click.option("--host",default="0.0.0.0")
@click.option("--port",default=int(os.getenv("PORT","8004")),type=int)
def main(host,port):
 ...
 card=AgentCard(name="grower-outreach-agent",description="Writes evidence-based personalized outbound email drafts.",supported_interfaces=[AgentInterface(protocol_binding="JSONRPC",url="http://"+host+":"+str(port)+"/")],version="1.0.0",default_input_modes=["text/plain"],default_output_modes=["text/plain"],capabilities=AgentCapabilities(streaming=True),skills=[AgentSkill(id="outreach",name="Personalized Outreach",description="Turns prospect evidence into email drafts.",tags=["grower","gtm","outreach"],examples=["Write outreach"])])
